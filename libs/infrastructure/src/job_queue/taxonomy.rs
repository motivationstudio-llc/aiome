...
impl KarmaTaxonomy {
    ...
    pub async fn classify(provider: &dyn LlmProvider, lesson: &str) -> Result<KarmaClassification, AiomeError> {
...
Constraint: Output ONLY raw JSON. No markdown blocks."#;
        ...
        match provider.complete(&prompt, Some(system_prompt)).await {
            Ok(response) => {
                ...
                serde_json::from_str::<KarmaClassification>(clean_json)
                    .map_err(|e| {
                        tracing::warn!("🧬 [Taxonomy] JSON Parse Error: {}. Raw: {}", e, response);
                        AiomeError::Infrastructure { reason: format!("Invalid classification format: {}", e) }
                    })
            },
            Err(e) => Err(e)
        }
