...
impl AiomeLogClient {
    pub async fn new(db_path: &str) -> Result<Self, AiomeError> {
        ...
        let _ = sqlx::query("ALTER TABLE logs ADD COLUMN hash TEXT").execute(&pool).await;

        Ok(Self { db: pool })
    }

    async fn get_last_hash(&self) -> String {
        let row: Option<(String,)> = sqlx::query_as("SELECT hash FROM logs WHERE hash IS NOT NULL ORDER BY id DESC LIMIT 1")
            .fetch_optional(&self.db)
            .await
            .unwrap_or(None);
        
        row.map(|r| r.0).unwrap_or_else(|| "AIOME_GENESIS_HASH_2026".to_string())
    }

    fn compute_hash(prev_hash: &str, event_type: &str, artifact: Option<&str>, path: Option<&str>, detail: Option<&str>) -> String {
...
impl AiomeLogger for AiomeLogClient {
    async fn log_success(&self, artifact_id: &str, output_path: &PathBuf) -> Result<(), AiomeError> {
        ...
        sqlx::query::<sqlx::Sqlite>("INSERT INTO logs (event_type, artifact_id, output_path, hash) VALUES (?, ?, ?, ?)")
            .bind("SUCCESS")
            .bind(artifact_id)
