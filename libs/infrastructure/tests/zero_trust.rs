...
async fn test_proxy_llm_provider_flow() {
    ...
    let provider = ProxyLlmProvider::new("http://127.0.0.1:9999".to_string(), "daemon".to_string(), "gemini".to_string());
...
async fn test_unauthorized_caller() {
    let provider = ProxyLlmProvider::new("http://127.0.0.1:9999".to_string(), "hacker".to_string(), "gemini".to_string());
