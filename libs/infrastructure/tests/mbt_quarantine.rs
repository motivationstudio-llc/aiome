use infrastructure::skills::{WasmSkillManager, UnverifiedSkill};
use std::env;
use std::fs;
use std::path::{Path, PathBuf};

/// Helper to set up a temporary environment representing the "Downloading" phase
fn setup_test_workspace(skill_name: &str, wasm_source: &Path, meta_json: &str) -> (WasmSkillManager, PathBuf) {
    ...
    // Write meta JSON
    fs::write(temp_dir.join(format!("{}.meta.json", skill_name)), meta_json).unwrap();
...
async fn test_mbt_quarantine_dry_run_fails() {
    ...
    assert!(result.is_err(), "Skill should reach Violated state and fail verification");
}
