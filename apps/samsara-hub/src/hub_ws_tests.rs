...
async fn test_ws_authentication_authorized_and_ping() {
    ...
    request.headers_mut().insert(axum::http::header::AUTHORIZATION, axum::http::HeaderValue::from_static("Bearer test_secret"));

    let (mut ws_stream, _) = connect_async(request).await.expect("Failed to connect");
    
    assert_eq!(state.active_connections.load(std::sync::atomic::Ordering::SeqCst), 1);

    // Send Ping
    use aiome_core::contracts::HubMessage;
    let ping = HubMessage::Ping { client_time: chrono::Utc::now().to_rfc3339() };
    ...
    assert_eq!(state.active_connections.load(std::sync::atomic::Ordering::SeqCst), 0);
}
