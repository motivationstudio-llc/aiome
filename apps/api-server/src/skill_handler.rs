...
pub async fn execute_forge_command(
    ...
    let _permit = state.forge_semaphore.acquire().await
        .map_err(|e| format!("Failed to acquire forge semaphore: {}", e))?;

    match command {
        "forge_skill" => {
            #[derive(serde::Deserialize)]
            struct ForgeReq { skill_name: String, initial_rust_code: String, description: String }
            match serde_json::from_str::<ForgeReq>(payload) {
                Ok(req) => {
                    // Security Gate: Validate skill name (Discovery A)
                    if req.skill_name.contains("..") || req.skill_name.contains('/') || req.skill_name.contains('\\') {
                        return Err("Security Violation: Invalid skill name (path traversal detected)".into());
                    }

                    match state.skill_forge.forge_skill(&req.skill_name, &req.initial_rust_code, 3, &req.description).await {
                        Ok(_) => {
                            Ok(format!("[forge_skill Result: Successfully compiled {}.wasm]", req.skill_name))
                        },
                        Err(e) => {
        ...
        "forge_test_run" => {
            #[derive(serde::Deserialize)]
            struct TestReq { skill_name: String, test_input: String }
            match serde_json::from_str::<TestReq>(payload) {
                Ok(req) => {
                    match state.wasm_skill_manager.dry_run_skill(&req.skill_name, &req.test_input).await {
                        Ok(true) => Ok("[forge_test_run Result: Skill verified successfully]".into()),
                        Ok(false) => Err("Skill failed security checklist (OOM or Format error)".into()),
        ...
        "forge_publish" => {
            #[derive(serde::Deserialize)]
            struct PublishReq { skill_name: String }
...
pub async fn execute_wasm_skill(
    ...
    match unverified.verify(&state.wasm_skill_manager).await {
        Ok(v) => {
            match state.wasm_skill_manager.call_skill(&v, "call", skill_input, None).await {
                Ok(res) => {
        ...
        Err(e) => {
            format!("[{} Error: Verification failed or Skill not found: {}]", skill_name, e)
        }
    }
}
