...
現在の Aiome は **OpenClaw との完全なハイブリッド統合（NAPI Bridge）を前提としたアーキテクチャ** です。
OpenClaw (Node.js/TypeScript) が「柔軟な認知と高速な推論のフロントエンド」を担い、Aiome (Rust) が「決して死なない堅牢なバックエンド、免疫システム (Karma)、セキュアな WASM スキル実行環境」としてその根底を支えます。

Aiome が提供するのは、AIの自由を奪うことではなく、**「OpenClaw が人間による24時間の監視なしで、30日間連続稼働し続けるための強固な規律（ガードレール）と進化の土台」** です。
...
- **Node.js**: `v20+` (OpenClaw との NAPI ハイブリッド連携に必須)
...
### 2. OpenClaw サンドボックス統合 (NAPI Hybrid)
Aiome は OpenClaw の自律推論と完全連携します。
- **NAPI-rs ビルド**: Aiome コアライブラリを Node.js ネイティブアドオンとしてビルドし、`packages/sentinel` 経由で OpenClaw にロードさせます。
- SQLite でのプロンプト隔離、Karma 抽出など、OpenClaw のすべての I/O と状態更新は Aiome の MPSC Channel を経由する構成（Actor Pattern）を推奨しています。
...
**OpenClawとのライセンス互換性について:**
OpenClaw (MITなど) のプラグインとして `sentinel` (NAPI Bridge) を組み込んで利用することは、社内利用・商用利用を問わず**完全に無償で合法**です。Aiome の ELv2 制限は、クラウド事業者等が Aiome そのものを「管理型サービス (Managed Aiome As A Service)」として第三者に販売するケースのみを防ぐものです。商用利用の詳細やライセンスに関するご相談は [motivationstudio, LLC](https://github.com/motivationstudio-llc/aiome) まで。
