...
1.  **The Sandbox (形式検証された絶対防衛網)**: 直接シェルを渡すのではなく、WASMコンテナや APIキーを物理隔離する Abyss Vault を通します。Aiome の隔離プロトコルは **TLA+ で数学的に証明（TLC Model Checker 検証済）** され、Rust の **Design by Contract (DbC)** を用いて「隔離の突破は論理的に不可能」であることをコンパイル時／ランタイムで強制します。「暴走してもホストを破壊しない」「データは絶対漏れない」という絶対の安心感を提供します。
...
| **Formal Verification** | TLA+ / TLC Model Checker / type-state DbC | 状態遷移と隔離プロトコルの絶対的・数学的な安全性証明 |
