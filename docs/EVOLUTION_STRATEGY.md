...
## 🏗️ 2つのプロダクト展開 (Product Strategy)

| バージョン | 名称 | 特徴 |
| :--- | :--- | :--- |
| **Standard** | **Aiome Core** | 基本的な自律ループ、教訓蓄積、免疫システム、および基本的な人格。 |
| **Enterprise** | **Aiome Pro** | 高度な並列処理、商用グレードの分析エンジン、および高度なカスタマイズ性。 |
...
1.  **Command Layer (Oracle LLM / Advanced APIs)**:
    - **役割**: システム操作、ジョブ管理、高度な論理推論。
    - **特徴**: 常にプロフェッショナルな助手として動作し、システムの信頼性を担保します。
2.  **Personality Layer (Local LLM / Ollama)**:
    - **役割**: ユーザーとの対話、人格の表現、日常的な支援。
    - **特徴**: 育成ステータスや蓄積された Karma に応じて、独自の振る舞いや口調を形成します。
